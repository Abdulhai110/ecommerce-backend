...
export class AuthController extends Controller{
    ...
    const url:string = `http://localhost:4200/user/reset/${token}`;
    ...
    // reset password
    async resetpassword(req:express.Request, res:express.Response, next:NextFunction){
        ...
        if(!user || (user && moment(user.ForgotpasswordExpires).isBefore(moment()))){
